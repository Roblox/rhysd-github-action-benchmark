...
function validateDataJson(data: DataJson) {
    ...
    const repoUrlMatcher = new RegExp(`^${serverUrl}/[^/]+/(rhysd-)?github-action-benchmark$`);
    const commitUrlMatcher = new RegExp(`^${serverUrl}/[^/]+/(rhysd-)?github-action-benchmark/commit/`);
